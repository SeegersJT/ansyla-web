	...
	return (
		<>
			<PageMeta
				...
				description={
					product?.description ??
					`${product?.name} — ${Utils.formatPrice(product?.price ?? 0, settings?.currency)} at ANSYLA Jewels.`
				}
				image={product?.images?.[0]?.url}
				path={`/product/${product?.product_no}`}
				type="product"
			/>
			<div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
				...
				<div className="grid gap-12 lg:grid-cols-2">
					...
					<div>
						...
						<div className="mt-8 flex items-center gap-4">
							<div className="flex items-center border border-border">
								<button
									...
									disabled={outOfStock}
									className="px-3.5 py-3 text-muted-foreground hover:text-primary disabled:cursor-not-allowed disabled:opacity-40"
									aria-label="Decrease"
								>
								...
								<button
									...
									disabled={outOfStock || selectedQuantity >= maxQuantity}
									className="px-3.5 py-3 text-muted-foreground hover:text-primary disabled:cursor-not-allowed disabled:opacity-40"
									aria-label="Increase"
								>
						...
						{!outOfStock && selectedQuantity >= maxQuantity && (
							<p className="mt-2 text-xs text-muted-foreground">
								Only {maxQuantity} {maxQuantity === 1 ? 'is' : 'are'} in stock —
								that's the most you can add.
							</p>
						)}
						...
						<div className="mt-5 flex flex-col gap-3 sm:flex-row">
							<button
								...
								disabled={outOfStock}
								className="flex-1 bg-gradient-gold py-3.5 text-xs font-medium uppercase tracking-luxe text-primary-foreground transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
							>
