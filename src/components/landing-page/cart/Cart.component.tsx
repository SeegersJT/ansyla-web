                ...
                <div className="grid gap-12 lg:grid-cols-[1fr_360px]">
                    ...
                    <aside className="h-fit border border-border bg-card p-6">
                        ...
                        <div className="mt-6 space-y-3 border-t border-border pt-6 text-sm">
                            <Row label="Subtotal" value={Utils.formatPrice(cartDataSubtotal, settings.currency)} />
                            {isCouponApplied && <Row label="Discount" value={`- ${Utils.formatPrice(discount, settings.currency)}`} />}
                            <Row
                                label="Shipping"
                                value={shippingCost === 0 ? "Free" : Utils.formatPrice(shippingCost, settings.currency)}
                            />
                            <div className="flex justify-between border-t border-border pt-3 font-serif text-xl">
                                <span>Total</span>
                                <span className="text-primary">{Utils.formatPrice(total, settings.currency)}</span>
                            </div>
                        </div>
