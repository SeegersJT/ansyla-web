    ...
    return (
        <div className="mx-auto grid max-w-7xl gap-12 px-4 py-16 sm:px-6 lg:grid-cols-2">
            <div className="mx-auto w-full max-w-md">
                ...
                <div className="mt-8 flex border border-border">
                    {(["login", "signup"] as const).map((mode) => (
                        <button
                            ...
                            className={`flex-1 py-3 text-xs uppercase tracking-luxe transition-colors ${selectedMode === mode ? "bg-gradient-gold text-primary-foreground" : "text-muted-foreground"
                                }`}
                        >
                ...
                <form onSubmit={onAccountSumbitClick} className="mt-6 space-y-4">
                    {selectedMode === "signup" && (
                        <FieldLabel label="Full Name" value={fullName} onChange={onFullNameChange} />
                    )}
                    <FieldLabel label="Email Address" type="email" value={email} onChange={onEmailChange} />
                    <FieldLabel label="Password" type="password" value={password} onChange={onPasswordChange} />
                    ...
                    >
                        {accountLoading ? "Please wait…" : selectedMode === "login" ? "Login" : "Create Account"}
                    </button>
                </form>
