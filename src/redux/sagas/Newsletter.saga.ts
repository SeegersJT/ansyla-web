...
import {
	NEWSLETTER_ACTIONS,
	requestSubscribeToNewsletterLoading,
...
function* handleSubscribeToNewsletterRequest(action: { type: string; payload: string }) {
	...
	try {
		yield call([firestoreService, firestoreService.add], 'Newsletters', {
...
export function* newsletterSaga() {
	yield takeLatest(
		NEWSLETTER_ACTIONS.REQUEST_SUBSCRIBE_TO_NEWSLETTER,
		handleSubscribeToNewsletterRequest
	)
}
